...
describe API::MergeRequests do
  ...
  describe "PUT /projects/:id/merge_requests/:merge_request_iid/merge_to_ref" do
    ...
    it "returns 406 if branch can't be merged" do
      ...
      expect(response).to have_gitlab_http_status(406)
      expect(json_response['message']).to eq("Branch cannot be merged to #{merge_request.merge_ref_path}")
    end

    it 'returns 401 if user has no permissions to read the merge request' do
      user2 = create(:user)
      project.add_reporter(user2)
